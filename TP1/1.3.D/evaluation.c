...
static Boolean unary_operator(Stack *s, int (*op)(int) ) {
  int operande = 0;
  int res = 0;
  
  if (isEmpty(s)){
    	fprintf(stderr,"Empty stack : can not proceed to the unary operation\n");
        return(FALSE);
  }
...
int racine_pf(int a) {
	if(a<0) {
		fprintf(stderr, "erreur racine carré d'un nombre < 0\n");
		return(FALSE);
	}
	return (int) sqrt(a);	
}	
...
Boolean racine(Stack *s) {
	return unary_operator(s, racine_pf);
}

Boolean cube(Stack *s) {
	return unary_operator(s, cube_pf);
}
