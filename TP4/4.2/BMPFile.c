...
void writeBMPFile(char * nomFichier, ImageRGB* image, int verbose) {
    ...
    FileHeader fh = {
        "BM",
        14+sizeof(DIBHeader)+image->height*image->width*sizeof(PixelRGB),
    "100",
        14+sizeof(DIBHeader)
    };
    ...
    fwrite((void *) fh.identity, sizeof(char), 2, fichier);
    fwrite( (void *) &(fh.file_size), sizeof(uint32_t), 1, fichier);
    fwrite((void *) fh.application_id, sizeof(char), 4, fichier);
    fwrite( (void *) &(fh.raster_address), sizeof(uint32_t), 1, fichier);
...
ImageRGB * readBMPFile( char *filename, int verbose) {
    ...
    FileHeader *fh = (FileHeader *) calloc(1, 14);
    if (fh==NULL) {
